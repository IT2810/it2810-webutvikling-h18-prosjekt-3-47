...
Testene våre er skrevet med Jest. Vi benytter oss av `react-test-renderer` sin `renderer` og `shallowRenderer`. For å teste på funksjoner eller state bruker vi `getInstance` på instansen generert av `renderer.create`. 

### Snapshot testing
Snapshot-testing er et nyttig verktøy for å forsikre seg om at brukergrensesnittet ikke endrer seg uventet. En typisk test case for en mobil-app rendrer brukergrensesnitt-komponenten, tar en dump av rendringen og sammenligner med et _"bilde"_ (ikke fysisk bilde, men gitt tilstand i et tidligere gitt tidspunkt) som er lagret sammen med testen. Testen mislykkes om de to _"bildene"_ ikke matcher. Hvis det ikke er gjort endringer i komponente, skal rendringen og _"bildet"_ matche. Hvis det er gjort endringer i komponenten, må snapshotet oppdateres. Dette kan gjøres ved å skrive `npm test -- -u`, hvor da npm starter `jest -u`. Snapshot testing er benyttet for alle hovedkomponenter og deres subkomponenter, med unntak av Geolocation.
...
### Todolist
Siden en del av logikken i TodoList avhenger av AsyncStorage, og vi ikke fikk til mocking av dette, har vi ikke fullstending testing av denne heller. Men vi fikk testet en del ved å manipulere og lese state, selv om dette ikke er helt best-practice.
...
### Geolocation
Vi fikk ikke tid til å skrive tester for denne, men hvis vi hadde hatt mer tid, kunne vi fått til en god del tester her. Vi kunne først testet små deler av koden, som `createMessage`, `getDistance`, `returnedFilteredArray`, `getPoints` osv ved å gi dem diverese gyldige og ugyldige inputs, og testet adferd. Vi kunne også testet `populateMarkers` for å teste at den genererer gyldig output i alle tilfeller. Deretter ville vi testet funksjoner som `isNearby`, som tar inn ett målpunkt og "nåværende posisjon", og sjekker avstanden med `getDistance` og endrer state hvis den finner ut at et nytt punkt er besøkt. Deretter kunne vi testet checkGoals som kjører `isNearby` på alle punktene i goals. Det kunne nok også gått an å mocke `Permission` og `Location` for å få skrevet tester for `getLocationAsync`, som tar seg av tillatelser og posisjonsinnhenting. 


#### Kilde: https://jestjs.io/docs/en/snapshot-testing

## Akseptansetesting
Som beskrevet tidligere er akseptansetester brukt som komplement der unit-testing ikke er tilfredstillende. Under følger beskrivelser av disse. Alle akseptansetestene ble gjennomført på flere Android-enheter, men vi fikk ikke tak i iOS-enhet for å gjennomføre denne testene i sin helhet på iOS.
...
Bildene 1-4 illustrerer hvordan det ser ut når vi beveger oss rundt på Gløshaugen, mens 5-6 illustrerer å ha besøkt flere punkter. Dette kan til en viss grad testes ved å øke `DISTANCE_THRESHOLD_FACTOR`, men dette er ikke et substitutt for å ta beina fatt og bli inspirert til å gå seg en tur. Vi plasserte punkter rundt på Gløshaugen for å verifisere at komponenten faktisk fungerte som den skulle, hvis man legger inn egne punkter som ligger tett, trenger man ikke å gå så langt for å få testa all adferd.
...
Vi navngir variabler og funksjoner ut ifra hva de gjør, og skriver kommentarer som sier noe om hva funksjonen gjør, hvilken type input og output er, samt hva de gjør i funksjonen.

Vi har en mappe med prosjekt-konstanter som inneholder farger, dager, layout og datoformat. Disse brukes i de forskjellige komponentene.
